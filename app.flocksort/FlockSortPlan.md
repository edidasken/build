...
- [x] FS-0-1: Create `New_Covenant/app.flocksort/flock_sort.html` — standalone PWA shell
- [x] FS-0-2: Create `New_Covenant/app.flocksort/flock_sort.css` — game-specific styles
- [x] FS-0-3: Create `New_Covenant/app.flocksort/flock_sort.js` — core game engine (ES module)
...
- [x] FS-1-1: Implement `Bottle` data model — `{ id, slots: Color[], capacity: 4 }`
- [x] FS-1-2: Implement `pour(fromId, toId)` — validates move rules, executes pour, returns undo snapshot
- [x] FS-1-3: Implement `isValidPour(from, to)` — rules: top color must match OR target empty, target must have space
- [x] FS-1-4: Implement `isSolved(bottles)` — every bottle is either empty or all-one-color
- [x] FS-1-5: Implement undo stack (max 50 moves) — `undoMove()` restores prior snapshot
- [x] FS-1-6: Implement `restartLevel()` — resets to initial level state from LEVELS data
- [x] FS-1-7: Implement `loadLevel(levelIndex)` — deep-clones level from LEVELS, initializes game state
- [x] FS-1-8: Write unit test suite (plain JS, no framework) in `app.flocksort/tests.js` — covers all engine functions
- [ ] FS-1-9: BCP

### Phase 2 — Level Pack (100 Launch Levels)
- [x] FS-2-1: Author 10 tutorial levels (3–4 bottles, 2 colors) — teaches mechanics
- [x] FS-2-2: Author 20 easy levels (4–5 bottles, 3 colors)
- [x] FS-2-3: Author 30 medium levels (5–6 bottles, 4–5 colors)
- [x] FS-2-4: Author 25 hard levels (7–8 bottles, 5–6 colors)
- [x] FS-2-5: Author 15 expert levels (9–12 bottles, 6–8 colors)
- [x] FS-2-6: Export all 100 levels as `LEVELS` array in `levels.js` — each entry: `{ id, name, bottles: [[...colors]], emptyBottles: N }`
- [x] FS-2-7: Validate all 100 levels are solvable (manual + engine dry-run)
- [ ] FS-2-8: BCP

### Phase 3 — UI & Animation
- [x] FS-3-1: Render bottle grid — CSS Grid, responsive (2–4 columns based on bottle count)
- [x] FS-3-2: Render each bottle as a fixed-size control with 4 stacked color segments (CSS `flex-direction: column-reverse`)
- [x] FS-3-3: Implement tap/click selection — first tap selects bottle (highlight ring), second tap pours
- [x] FS-3-4: Pour animation — CSS `transform: translateY` + `opacity` transition, ~300ms, ease-out
- [x] FS-3-5: Liquid fill animation — segments animate height from 0 to full on arrival
- [x] FS-3-6: Wobble/shake animation on invalid pour attempt
- [x] FS-3-7: Level-complete celebration — `confetti` burst (pure CSS + JS, no library) + "Well Done!" overlay
- [x] FS-3-8: Color palette — 12 named game colors, all WCAG AA compliant, distinct in both normal and color-blind modes
- [x] FS-3-9: Bottle count auto-scales layout — 4 bottles: 2×2, 6: 3×2, 8: 4×2, 10: 5×2, 12: 4×3
- [ ] FS-3-10: BCP

### Phase 4 — App Shell & Navigation
- [x] FS-4-1: Top bar — "FlockSort" wordmark + level number + move counter
- [x] FS-4-2: Action buttons — Undo (↩), Restart (↺), Menu (☰) — always visible, no coins required
- [x] FS-4-3: Level select screen — scrollable grid of level cards (numbered, shows star rating if solved)
- [x] FS-4-4: Settings panel — color-blind mode toggle, sound toggle, theme select
- [x] FS-4-5: Menu button routes to level select; deep-link support (`?level=42`)
- [x] FS-4-6: Apply `new_covenant.css` theming — `var(--bg)`, `var(--bg-raised)`, `var(--ink)`, `var(--gold)` for chrome; game colors are fixed/not theme-dependent
- [ ] FS-4-7: BCP

### Phase 5 — Progress & Persistence (Local-First)
- [x] FS-5-1: Save progress to `localStorage` — `flocksort_progress`: `{ levelsUnlocked, levelStars: {}, totalMoves }`
- [x] FS-5-2: Keep all 100 levels available from level select — no coins, no gates, with progress still tracked
- [x] FS-5-3: Star rating per level — 3 stars (≤ optimal moves), 2 stars (≤ 1.5× optimal), 1 star (solved)
- [x] FS-5-4: Total stats view — levels solved, total moves, completion percentage
- [x] FS-5-5: "Continue" button on home — resumes last-played level
...
- [x] FS-6-1: Pour sound — short liquid gurgle, synthesized via Web Audio API (no audio files, no network requests)
- [x] FS-6-2: Complete sound — short pleasant chime, Web Audio API
- [x] FS-6-3: Invalid move — subtle click/buzz, Web Audio API
- [x] FS-6-4: All sounds gated by `settings.soundEnabled` (localStorage) — default OFF
- [ ] FS-6-5: BCP
